...
import { getClinicOperationsContext, getClinicWaitlistOfferSlots, markAppointmentCompleted, markAppointmentNoShow, offerWaitlistSlot } from '../../src/lib/phase3'
...
export default function ProfessionalOperationsScreen() {
  ...
  const complete = async (id: string) => {
    setBusyId(id); setMessage(null)
    try { await markAppointmentCompleted(id); setMessage(t('visitCompleted')); await operations.refetch() } catch { setMessage(t('operationFailed')) } finally { setBusyId(null) }
  }
  const offer = async () => {

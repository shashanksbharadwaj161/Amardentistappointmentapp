...
export function VerificationWorkspace({ query }: { query: string }) {
  ...
  const [details, setDetails] = useState<VerificationDetails>({ evidence: [], history: [] })
  ...
  useEffect(() => {
    if (!selected) return
    let active = true
    void Promise.resolve().then(() => { if (active) setDetailsLoading(true); return loadVerificationDetails(selected) }).then((next) => { if (active) setDetails(next) })
      .catch(() => { if (active) setMessage('Private evidence could not be loaded.') })

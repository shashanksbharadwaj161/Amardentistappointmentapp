  ...
  return (
    <div className="console">
      <aside ref={sidebarRef} id="admin-navigation" role={mobileNavigation ? 'dialog' : undefined} aria-label={mobileNavigation ? 'Navigation menu' : undefined} className={menuOpen ? 'sidebar sidebar-open' : 'sidebar'} aria-hidden={mobileNavigation && !menuOpen} inert={mobileNavigation && !menuOpen ? true : undefined} aria-modal={mobileNavigation && menuOpen ? true : undefined} tabIndex={mobileNavigation ? -1 : undefined} onKeyDown={containNavigationFocus}>
        ...
        <nav aria-label="Primary navigation">{nav.map(({ id, label, icon: Icon }) => { const enabled = id === 'overview' || id === 'verification'; const active = activeView === id; return <button key={label} className={active ? 'nav-item nav-active' : 'nav-item'} type="button" aria-current={active ? 'page' : undefined} disabled={!enabled} title={!enabled ? `${label} becomes available in a later phase` : undefined} onClick={() => { if (enabled) { setActiveView(id as 'overview' | 'verification'); closeMenu() } }}><Icon /><span>{label}</span></button> })}</nav>

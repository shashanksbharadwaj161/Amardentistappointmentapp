...
function Console({ identity, onSignOut }: { identity: AdminIdentity; onSignOut: () => void }) {
  ...
  const [activeView, setActiveView] = useState<'overview' | 'verification' | 'configuration'>('overview')
  ...
  return (
    <div className="console">
      <aside ref={sidebarRef} id="admin-navigation" role={mobileNavigation ? 'dialog' : undefined} aria-label={mobileNavigation ? 'Navigation menu' : undefined} className={menuOpen ? 'sidebar sidebar-open' : 'sidebar'} aria-hidden={mobileNavigation && !menuOpen} inert={mobileNavigation && !menuOpen ? true : undefined} aria-modal={mobileNavigation && menuOpen ? true : undefined} tabIndex={mobileNavigation ? -1 : undefined} onKeyDown={containNavigationFocus}>
        ...
        <nav aria-label="Primary navigation">{nav.map(({ id, label, icon: Icon }) => { const enabled = id === 'overview' || id === 'verification' || (id === 'configuration' && isSuperAdmin); const active = activeView === id; return <button key={label} className={active ? 'nav-item nav-active' : 'nav-item'} type="button" aria-current={active ? 'page' : undefined} disabled={!enabled} title={!enabled ? `${label} requires a later phase or Super Admin access` : undefined} onClick={() => { if (enabled) { setActiveView(id as 'overview' | 'verification' | 'configuration'); closeMenu() } }}><Icon /><span>{label}</span></button> })}</nav>
      ...
      <div className="workspace" inert={mobileNavigation && menuOpen ? true : undefined}>
        <header className="topbar">
          ...
          <div className="search" role="search"><Search /><input aria-label="Search verification applications" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder={activeView === 'verification' ? 'Search applications' : 'Search verification queue'} /></div>
        ...
        <main className="content">{activeView === 'verification' ? <VerificationWorkspace query={searchQuery} /> : activeView === 'configuration' ? <FinanceConfiguration /> : <>

...
select is((select amount_bdt from public.prepare_payment_checkout('55000000-0000-4000-8000-000000000001','bkash','patient-key-1234')),300::numeric,'checkout amount is derived from trusted appointment deposit');
...
select ok((select count(*)=2 and count(*) filter(where entry_type='payment' and amount_bdt=300)=1 and count(*) filter(where entry_type='commission' and amount_bdt=-15)=1 from public.clinic_ledger_entries),'confirmed payment reconciles gross and commission exactly once');
...
select is((select on_hand from public.inventory_lots where id=current_setting('test.lot')::uuid),6::numeric,'on hand stock matches movements');
